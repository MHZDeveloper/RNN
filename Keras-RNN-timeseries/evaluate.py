...
def matricise_dataset(dataset, look_back=1, batch_size=1):
    n = int(math.floor((len(
        dataset) - look_back - 1) / batch_size)) * batch_size  # the number of samples must be divisible by the batch size
    dataX, dataY = [], []
    for i in range(n):
...
def print_results(mse_vals):
    ...
    print('Best mse with a GRU recurrent neural network was ' + str(gru_mse_min) + ' with a look back of ' + str(
        gru_mse_argmin))
    print('Best mse with a LSTM recurrent neural network was ' + str(lstm_mse_min) + ' with a look back of ' + str(
        lstm_mse_argmin))
    print('Best mse with a linear regression model was ' + str(regr_mse_min) + ' with a look back of ' + str(
        regr_mse_argmin))
